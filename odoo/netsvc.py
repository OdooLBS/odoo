    ...
    def __init__(self):
        ...
        if tools.config["log_db"] != "%d":
            with contextlib.suppress(Exception), tools.mute_logger(
                "odoo.sql_db"
            ), sql_db.db_connect(tools.config["log_db"], allow_uri=True).cursor() as cr:
                cr.execute(
...
def init_logger():
    ...
    warnings.filterwarnings(
        "ignore",
        r"the load_module\(\) method is deprecated",
        category=DeprecationWarning,
    ...
    warnings.filterwarnings(
        "ignore", r"invalid escape sequence", category=SyntaxWarning, module=".*vobject"
    )
    ...
    if tools.config["syslog"]:
        ...
        log_dir = os.path.dirname(tools.config["logfile"])
        try:
